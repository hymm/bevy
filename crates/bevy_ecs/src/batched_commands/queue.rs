use crate::{
    batched_commands::commands::BatchedCommand,
...
struct CommandMeta {
    ...
    consume_command_and_get_size: unsafe fn<'a>(
        value: OwningPtr<'a, Unaligned>,
        world: Option<NonNull<World>>,
        cursor: &mut usize,
    ) -> &'a mut dyn BatchedCommand,
}
...
impl CommandQueue {
    ...
    #[inline]
    pub fn push(&mut self, command: impl BatchedCommand) {
...
impl RawCommandQueue {
    ...
    #[inline]
    pub unsafe fn push<C: BatchedCommand>(&mut self, command: C) {
        ...
        #[repr(C, packed)]
        struct Packed<C: BatchedCommand> {
            meta: CommandMeta,
            command: C,
        }
