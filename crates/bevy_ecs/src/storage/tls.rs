...
impl ThreadLocals {
    ...
    unsafe fn initialize_resource_internal(&mut self, id: ComponentId) -> &mut ResourceData<false> {
        self.storage
            .initialize_with(id, &self.info, || ArchetypeComponentId(id.index()))
    }
...
// TODO: can remove this trait if `bevy_app` and `bevy_ecs` are merged into one crate
...
impl Drop for ThreadLocalStorage {
    fn drop(&mut self) {
        TLS.with_borrow_mut(|tls| mem::replace(tls, ThreadLocals::new()));
    }
}
