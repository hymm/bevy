use bevy_ptr::{IsAligned, OwningPtr};
...
impl DynamicBundle {
    ...
    pub fn add<T: 'static + IsAligned + Component>(&mut self, mut data: T) {
        match self.indices.entry(TypeId::of::<T>()) {
            Entry::Occupied(occupied_entry) => {
                ...
                let (_info, offset) = &self.info[index];

                // Safety: `offset` is garunteed to be in `data`
                let ptr = unsafe { self.data.byte_add(*offset) };
                unsafe {
                    // TODO: should maybe call the drop from the ComponentDescriptor instead?
                    // In most other situations we won't know the type when dropping
                    OwningPtr::<T>::new(ptr).drop_as::<T>();
                }
            ...
            Entry::Vacant(vacant) => {
                ...
                let align_offset =
                    unsafe { self.data.byte_add(self.cursor) }.align_offset(layout.align());
                let offset = self.cursor + align_offset;
