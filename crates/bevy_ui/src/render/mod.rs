...
use bevy_render::render_phase::{PhaseItemExtraIndex, ViewSortedRenderPhases};
...
fn extract_shadows(
    ...
    for (uinode, shadow, view_visibility, clip, camera) in uinode_query.iter() {
        ...
        let position = uinode.position() + shadow.offset - 0.5 * shadow.spread_radius;
...
pub fn extract_default_ui_camera_view(
    ...
    mut live_entities: Local<EntityHashSet<Entity>>,
) {
    live_entities.clear();
    ...
    ) in uinode_query.iter()
    {
        ...
        } in &text_layout_info.glyphs
        {
            if *section_index != current_section {
                color = text.sections[*section_index].style.color.to_linear().into();
...
pub fn queue_uinodes(
    ...
    for (entity, extracted_uinode) in extracted_uinodes.uinodes.iter() {
        ...
        let pipeline = match extracted_uinode.instance {
            ExtractedInstance::Node(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::Text(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::LinearGradient(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::RadialGradient(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::DashedBorder(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::CNode(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::CText(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::CLinearGradient(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::CRadialGradient(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::CDashedBorder(..) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::Shadow(_) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
            ...
            ExtractedInstance::CShadow(_) => pipelines.specialize(
                &pipeline_cache,
                &ui_pipeline,
                UiPipelineKey {
                    hdr: view.hdr,
...
pub fn prepare_uinodes(
    ...
    if let Some(view_binding) = view_uniforms.uniforms.binding() {
        ...
        for (_, ui_phase) in &mut phases.0 {
